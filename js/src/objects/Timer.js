...
class Timer {
  ...
  update() {
    ...
    var y =
      this.scene.yeti.body.y - window.innerHeight / 2 + this.timerText.height;
    var timeLeft =
      Math.floor(this.scene.levelTime / 1000) -
      Math.floor(
        (this.scene.timer.getProgress() * this.scene.levelTime) / 1000
      );
